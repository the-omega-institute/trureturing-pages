...
UI = (ASSETS / 'millennium.mjs').read_text().replace("import { t, localizeData, ready, canonicalText } from './i18n.mjs';", '')
...
# This legacy component harness uses English identity localization. Real bilingual
# module loading, History and Storage are covered by tests/browser/i18n.cjs.
PRELUDE = '''
const ready = Promise.resolve();
const localizeData = value => value, canonicalText = value => value;
const t = (text, ...values) => values.length ? text.replace(/\\{(\\d+)\\}/g, (m,i) => values[i] ?? m) : text;
...
def run():
    ...
    with sync_playwright() as pw:
        ...
        page.locator('[data-mm-node="5040"]').click();expect(page.locator('#mm-detail')).to_contain_text('10080 instance')
        ...
        page.get_by_role('button',name='Add to watchlist',exact=True).click()
        ...
        page.locator('#mm-detail').get_by_role('button',name='Export watchlist').click()
        ...
        expect(page.locator('#mm-detail h2')).to_contain_text('n≥5041');expect(page.locator('#mm-detail')).to_contain_text('RH ⇒ this statement: Unassessed')
        passed('individual specification route expands its own DAG node and direction states')
        page.get_by_role('button',name='Equivalence catalogue',exact=True).click();page.locator('#mm-family').select_option('')
        expect(page.locator('.mm-spec-card')).to_have_count(93)
        page.locator('#mm-search').fill('A080');expect(page.locator('.mm-spec-card')).to_have_count(1);expect(page.locator('.mm-spec-card')).to_contain_text('Preprint: verification pending')
        page.locator('#mm-search').fill('no-such-formulation');expect(page.locator('.mm-spec-card')).to_have_count(0)
        expect(page.locator('.mm-empty')).to_contain_text('No matches.')
        passed('all 93 specifications, live search, preprint warning and empty result')
        page.get_by_role('button',name='Shared core',exact=True).click()
        expect(page.locator('[data-mm-node="zeros"].mm-common')).to_be_attached();expect(page.locator('.mm-reuse>button')).to_have_count(9)
        page.get_by_text('Inspect convergence of the structural closure',exact=True).click()
        expect(page.locator('.mm-content')).to_contain_text('not a fixed-point proof for RH, an operator or a dynamical system')
        ...
        for pid in ['pnp','hodge','navier-stokes','yang-mills','bsd','poincare']:
            ...
            expect(page.locator('.mm-warning')).to_contain_text('starter map');expect(page.locator('.mm-stats strong').first).to_have_text('0')
        expect(page.locator('.mm-science')).to_contain_text('Mathematical literature: solved');expect(page.locator('#mm-detail')).to_contain_text('Repository not inventoried')
        ...
        bad=mount(payload={'schema':'wrong'});expect(bad.get_by_text('Research map unavailable',exact=True)).to_be_visible();bad.close()
        bad_entry=mount(payload={'schema':'wrong'},entry=True);expect(bad_entry.locator('#millennium-entry')).to_contain_text('The catalogue is temporarily unavailable');expect(bad_entry.get_by_text('Original released dossier')).to_be_visible();bad_entry.close()
        passed('malformed data fails closed and leaves original dossier content intact')
        blocked=mount(blocked=True);expect(blocked.locator('.mm-heading h1')).to_have_text('Riemann Hypothesis');expect(blocked.locator('#mm-message')).to_contain_text('unavailable');blocked.close()
