...
def render_news(output, snapshot, shell):
    ...
    for item in catalog["results"]:
        ...
        body = f'''<article class="news-result" id="{item['id']}"><div class="news-result-meta"><time datetime="{item['date']}">{item['date']}</time><span class="news-status {item['kind']}">{item['kind'].capitalize()} in Lean</span></div><div><p class="eyebrow">{esc(item['field'])}</p><h3>{esc(item['title'])}</h3><p>{esc(item['summary'])}</p><p class="news-release-state">{release_state}</p><div class="news-links"><a href="{BOOK}Blueprint/{module}.html">Read dossier <i data-lucide="arrow-up-right"></i></a><a href="{REPO}/pull/{item['pr']}">Merged proof #{item['pr']}</a><a href="{esc(item['source_url'])}">Original question</a></div><details><summary>Exact scope &amp; proof record</summary><p>{esc(item['scope'])}</p><p><a href="{REPO}/blob/{commit}/{module}.lean">{esc(module + '.' + item['declaration'])}</a></p><p><a href="{evidence}">Frozen module record</a> <code>{item['statement_id']}</code></p><small>Evidence snapshot: {commit}. The linked mdBook follows upstream development.</small></details></div></article>'''
        results.append(body)
