...
def parse_problem(text: str, filename: str) -> dict:
    ...
    return {"slug": slug, "title": title, "bibkey": str(meta.get("bibkey", "")), "arxiv_id": meta["arxiv_id"], "triage": meta["triage"], "motivation_gids": gids, "sections": content, "source_digest": digest(text.encode()), "literature_status": "not-rechecked", "last_literature_check": None, "route_status": "proposed"}
...
def page_shell(title: str, root: str, body: str, active="Research") -> str:
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#090c10"><title>{esc(title)} | trureturing</title>
