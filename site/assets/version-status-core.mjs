...
export function renderStatus(value, { now = Date.now(), translate = t } = {}) {
  ...
  return `<section class="version-notice ${unavailable || fallback || value.halt ? 'needs-attention' : 'is-current'}" role="status"><p>${escape(notice)}</p></section>
    <ol class="version-stages" aria-label="${text('Five release stages')}">${STAGES.map(stage => `<li data-stage="${stage}"${stop === stage ? ' aria-current="step"' : ''}><span>${LABELS[stage]}${stage === 'deployed' ? ' <small>(live)</small>' : ''}</span><strong>${number(value.counts[stage])}</strong></li>`).join('')}</ol>
